...
const Card: React.FC<{ children: React.ReactNode; className?: string; title?: string }> = ({ children, className = '', title }) => (
  <div className={`panel-surface rounded-xl transition-all duration-500 ${className}`}>
    {title && (
...
export default function App() {
  ...
  return (
    <div className="h-screen flex flex-col lg:flex-row bg-app text-zinc-100 overflow-hidden font-sans">
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-app pb-24 lg:pb-0 overflow-hidden relative">
        <header className="px-8 h-20 border-b border-white/5 flex justify-between items-center bg-black/20 backdrop-blur-xl z-30">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-8 lg:p-12 custom-scroll scroll-smooth">
            ...
            {activeStep === 'preview' && (
              <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 h-full flex flex-col">
                <Card className="flex-1 flex flex-col min-h-0">
                  <div className="flex-1 mono text-[11px] text-zinc-400 bg-black/40 p-6 sm:p-10 rounded-xl border border-white/5 overflow-y-auto custom-scroll leading-[1.8] whitespace-pre-wrap selection:bg-orange-500/20">
            ...
            {activeStep === 'sandbox' && (
              <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 h-full flex flex-col min-h-0">
                <Card className="flex-1 flex flex-col overflow-hidden min-h-0">
                  <div ref={chatScrollRef} className="flex-1 overflow-y-auto space-y-8 pr-4 custom-scroll pb-6 scroll-smooth">
                    {messages.length === 0 && (
                      <div className="h-full flex flex-col items-center justify-center text-zinc-700 mt-20 opacity-40">
                        ...
                        <p className="text-[10px] uppercase tracking-[0.6em] font-black">Neural Sandbox Initialization Required</p>
                      </div>
                    )}
                    {messages.map((m, i) => (
      ...
      <footer className="fixed bottom-0 left-0 right-0 h-14 px-10 border-t border-white/5 bg-black/40 backdrop-blur-3xl flex justify-between items-center z-50 text-[10px] mono text-zinc-600 font-bold">
